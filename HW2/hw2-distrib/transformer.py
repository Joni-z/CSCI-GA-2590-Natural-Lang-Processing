...
class Transformer(nn.Module):
    def __init__(self, vocab_size, num_positions, d_model, d_internal, num_classes, num_layers):
        ...
        self.pos_encoding = PositionalEncoding(d_model, num_positions, batched=False)
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, d_internal):
        ...
        self.ffn1 = nn.Linear(d_model, d_internal)
        self.ffn2 = nn.Linear(d_internal, d_model)

    def forward(self, input_vecs):
        # input_vecs: (seq_len, d_model)
        ...
        scores = torch.mm(Q, K.t()) / (self.d_internal ** 0.5)  # (seq_len, seq_len)
        attn_weights = torch.softmax(scores, dim=1)
        attn_output = torch.mm(attn_weights, V)  # (seq_len, d_model)
        # (2) Residual
...
def train_classifier(args, train, dev):
    import os
    vocab_size = 27  # lowercase a-z + space
    num_positions = 20
    num_classes = 3
    d_model = 64
    d_internal = 32
    # Q1/default: 2 layers. For Q3 (more layers): set env NUM_LAYERS=4 when running.
    ...
    for t in range(num_epochs):
        ...
        print("Epoch %i loss: %f" % (t, loss_this_epoch / len(train)))
    model.eval()
    return model
